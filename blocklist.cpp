...
int BlockList::FindPairs(char *first_) {
    std::fstream file;
    file.open(FileName);
    Block search;
    file.seekg(Start);
    ...
    while (1) {
        if ((strcmp(first_, search.MaxValue) <= 0) && (strcmp(first_, search.MinValue) >= 0) &&
            ...
            int index = std::lower_bound(search.first_array, search.first_array + search.CurrentSize, first_,
                                         compare_char) - search.first_array;
            if (strcmp(search.first_array[index], first_) == 0 && index < search.CurrentSize) {
                location = search.second_array[index];
                break;
                // index++;
            }
        }
...
std::vector<int> BlockList::FindAllPairs(char *first_) {
    std::fstream file;
    file.open(FileName);
    Block search;
    file.seekg(Start);
    ...
    while (1) {
        if ((strcmp(first_, search.MaxValue) <= 0) && (strcmp(first_, search.MinValue) >= 0) &&
            ...
            while (strcmp(search.first_array[index], first_) == 0 && index < search.CurrentSize) {
                values.push_back(search.second_array[index]);//自动带有ISBN的升序
                index++;
            }
        }
        if (strcmp(first_, search.MinValue) < 0)break;
...
void BlockList::ShowAllBooks() {
    std::fstream file, bookfile;
    file.open("ISBN_BookList");
    file.seekg(Start);
    Block read;
    file.read(reinterpret_cast<char *>(&read), sizeof(Block));
    if (read.CurrentSize == 0 && read.Next == -100000) {
        std::cout << "\n";
        return;
    }
    while (1) {
        bookfile.open("BookFile");
        if (read.CurrentSize != 0) {
            for (int i = 0; i < read.CurrentSize; ++i) {
                bookfile.seekg(read.second_array[i]);
                book to_show;
                bookfile.read(reinterpret_cast<char *>(&(to_show.info)), sizeof(bookInfo));
                to_show.ShowABook();
            }
        }
        bookfile.close();
        if (read.Next != -100000) {
            file.seekg(read.Next);
            file.read(reinterpret_cast<char *>(&read), sizeof(Block));
        } else break;
    }
    file.close();
